...
@Tag(name = "Ranking V1 API", description = "레짐별 랭킹 API 입니다. 돌파시작/돌파준비/돌파실패 3개 레짐에 대해서만 랭킹을 제공합니다.")
public interface RankingV1ApiSpec {
  // ===================== HTTP (스냅샷) =====================

  @Operation(
      summary = "돌파시작 랭킹 스냅샷 조회",
      description = """
          BREAKOUT_START 레짐 종목을 모멘텀(1차 필터) → FIP(2차 필터) 기준으로 정렬한 상위 50개를 반환합니다.
          초기 페이지 로딩 시 호출하여 현재 스냅샷을 수신하고, 실시간 변경분은 SSE로 구독하세요.
          """
  )
  ApiResponse<BreakoutStartResponse> getBreakoutStartRanking();
  ...
  @Operation(
      summary = "돌파실패 랭킹 스냅샷 조회",
      description = """
          BREAKOUT_FAILED 레짐 종목을 모멘텀(1차 필터) → FIP(2차 필터) 기준으로 정렬한 상위 50개를 반환합니다.
          초기 페이지 로딩 시 호출하여 현재 스냅샷을 수신하고, 실시간 변경분은 SSE로 구독하세요.
          """
  )
  ApiResponse<BreakoutFailedResponse> getBreakoutFailedRanking();
}
