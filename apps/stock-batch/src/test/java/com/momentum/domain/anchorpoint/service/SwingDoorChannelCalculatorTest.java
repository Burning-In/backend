...
class SwingDoorChannelCalculatorTest {
  ...
  private final SwingDoorChannelCalculator calculator = new SwingDoorChannelCalculator(
      new StockAnchorPointSlopeCalculator());

  private final Stock stock = Stock.of("테스트종목", "000500", StockRegime.UNKNOWN, StockTrend.OTHER);
  private final StockAnchorPoint anchorPoint = StockAnchorPoint.create(ANCHOR_PRICE, 1_000L, ANCHOR_DATE, stock);
  ...
  void narrowChannel_withLooserSlope_keepsAccumulatedChannel() {
    // given
    StockAnchorPointCalculation lastCalculation = calculationOf(DAY1_UPPER, DAY1_LOWER);
  ...
  void narrowChannel_withAccumulatedUpperBound_detectsOutOfChannel() {
    // given
    StockAnchorPointCalculation lastCalculation = calculationOf(DAY1_UPPER, DAY2_LOWER);
  ...
  void narrowChannel_withoutAccumulation_missesOutOfChannel() {
    // given
    StockAnchorPointCalculation lastCalculation = calculationOf(DAY2_UPPER, DAY2_LOWER);
  ...
  private StockAnchorPointCalculation calculationOf(String slopeUpperMax, String slopeLowerMin) {
    return StockAnchorPointCalculation.create(DAY2_CLOSE, 1_000L, ANCHOR_DATE.plusDays(2),
