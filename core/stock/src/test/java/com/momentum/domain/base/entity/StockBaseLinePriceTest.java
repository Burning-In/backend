...
class StockBaseLinePriceTest {
  ...
  @Test
  @DisplayName("가격 차이가 임계치(%) 이하이면 임계 범위 안이다")
  void withinThreshold() {
    ...
    assertThat(linePrice.isWithinThreshold(withinPrice, thresholdPercent)).isTrue();
  }

  @Test
  @DisplayName("가격 차이가 임계치(%)를 초과하면 임계 범위 밖이다")
  void outsideThreshold() {
    ...
    assertThat(linePrice.isWithinThreshold(outsidePrice, thresholdPercent)).isFalse();
  }
}
